...
module.exports = {
	...
	run: async (client, interaction) => {
        ...
        if(worker.status === 'Break') {
            ...
            worker.breakTime += (worker.afkDates.afkOut[worker.afkDates.afkOut.length - 1] - worker.afkDates.afkIn[worker.afkDates.afkOut.length - 1]) / 1000 / 60 / 60;
        }

        worker.worked = (Date.now() - msg.createdTimestamp) / 1000 / 60 / 60 - worker.breakTime;
       
        const hours_worked = Math.floor(worker.worked);
        const minutes_worked = Math.floor((worker.worked - hours_worked) / 0.01666667);
        
        const hours_break = Math.floor(worker.breakTime);
        const minutes_break = Math.floor((worker.breakTime - hours_break) / 0.01666667);
        ...
        const minutes_weekly = Math.floor((worker.weeklyWorked - hours_weekly) / 0.01666667);
        
        const hourlyRate = role.hourlySalary[worker.experience] || role.hourlySalary.Mid;
        salary = Math.floor((worker.worked * hourlyRate) * 100) / 100;
        ...
        function dateFormat(timestamp) {
            ...
            const timeParts = Object.fromEntries(londonTime.map(({ type, value }) => [type, value]));
		...
		await afkPresences.deleteOne({ guildId: interaction.guild.id, userId: userId })
